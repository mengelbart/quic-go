...
var _ = Describe("Key Update tests", func() {
	...
	runServer := func() {
		var err error
		server, err = quic.ListenAddr("localhost:0", getTLSConfig(), &quic.Config{Tracer: &simpleTracer{}})
	...
	It("updates the key once after the handshake", func() {
		runServer()
		...
		Expect(keyPhasesSent).To(Equal(2))
		Expect(keyPhasesReceived).To(Equal(2))
	})
	...
	It("downloads a large file", func() {
		...
		Expect(keyPhasesReceived).To(BeNumerically(">", 5))
