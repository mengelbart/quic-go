...
func setReceiveBuffer(c net.PacketConn, logger utils.Logger) {
	...
	if size >= protocol.DesiredReceiveBufferSize {
		logger.Debugf("Conn has receive buffer of %d kB (wanted: at least %d kB)", size/1024, protocol.DesiredReceiveBufferSize/1024)
	}
	...
	if newSize < protocol.DesiredReceiveBufferSize {
		log.Printf("Failed to sufficiently increase receive buffer size. Was: %d kb, wanted: %d kb, got: %d kb.", size/1024, protocol.DesiredReceiveBufferSize/1024, newSize/1024)
		return
	}
	logger.Debugf("Increased receive buffer size to %d kb", newSize/1024)
}
